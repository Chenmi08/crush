...
// Stats holds model usage statistics for a single user turn, aggregated
// over every model step of that turn: time to first token, generation
// speed, and token accounting. It is persisted as a message part so the
// per-turn footer survives session restores.
type Stats struct {
	// TTFTMillis is the time to first token of the turn's first step.
	TTFTMillis int64 `json:"ttft_ms,omitempty"`
	// TokensPerSecond is the turn's weighted generation speed: total
	// output tokens over the total generation window.
	TokensPerSecond float64 `json:"tokens_per_second,omitempty"`
	// CacheReadTokens is the sum of prompt tokens served from the
	// provider's prompt cache across the turn's steps.
	CacheReadTokens int64 `json:"cache_read_tokens,omitempty"`
	// CacheCreationTokens is the sum of prompt tokens written to the
	// provider's prompt cache across the turn's steps.
	CacheCreationTokens int64 `json:"cache_creation_tokens,omitempty"`
	// TotalPromptTokens is the sum of every step's prompt tokens,
	// including input, cache reads, and cache creation. It is the
	// denominator of the cache hit rate and the increment of the
	// session's cumulative prompt counter for this turn.
	TotalPromptTokens int64 `json:"total_prompt_tokens,omitempty"`
	// OutputTokens is the sum of tokens generated by all the turn's
	// steps.
	OutputTokens int64 `json:"output_tokens,omitempty"`
	// Estimated is true when any step's usage was approximated locally
...
// TTFT returns the time to first token as a duration.
func (s Stats) TTFT() time.Duration {
	return time.Duration(s.TTFTMillis) * time.Millisecond
}
...
func (s Stats) CacheHitRate() float64 {
	...
	return float64(s.CacheReadTokens) / float64(s.TotalPromptTokens)
}

// IsZero reports whether the stats carry no displayable values.
func (s Stats) IsZero() bool {
	return s.TTFTMillis == 0 && s.TokensPerSecond == 0 &&
		s.CacheReadTokens == 0 && s.CacheCreationTokens == 0 &&
		s.TotalPromptTokens == 0 && s.OutputTokens == 0
}
...
// Stats returns the per-turn usage statistics part, if any.
func (m *Message) Stats() (Stats, bool) {
	for _, part := range m.Parts {
		if s, ok := part.(Stats); ok {
			return s, true
		}
	}
	return Stats{}, false
...
// SetStats replaces the per-turn usage statistics part, appending one
// when the message has none yet.
func (m *Message) SetStats(stats Stats) {
	for i, part := range m.Parts {
		if _, ok := part.(Stats); ok {
			m.Parts[i] = stats
			return
		}
	}
	m.Parts = append(m.Parts, stats)
}
