...
// StepStats holds runtime-only statistics for the most recent model step
// of a session: time to first token, generation speed, and cache usage.
// These values are never persisted; the service keeps them in memory.
type StepStats struct {
	TTFT                time.Duration
	TokensPerSecond     float64
	CacheReadTokens     int64
	CacheCreationTokens int64
	// TotalPromptTokens is the total number of input tokens for the
	// step, including input, cache reads, and cache creation. It is the
	// denominator of CacheHitRate.
	TotalPromptTokens int64
	// CacheHitRate is the fraction (0..1) of prompt tokens served from
	// the provider's prompt cache.
	CacheHitRate float64
	// OutputTokens is the number of tokens generated by the step.
	OutputTokens int64
}

// SessionTokens holds cumulative token usage for an entire session.
// Unlike StepStats, these counters are persisted with the session and
// survive restarts.
...
type Session struct {
	...
	CreatedAt        int64
	UpdatedAt        int64
	// Stats carries runtime-only step statistics. It is repopulated by
	// the service on reads and must not be persisted.
	Stats StepStats
...
type service struct {
	*pubsub.Broker[Session]
	...
	// Step stats stay in memory for the same reason: they describe the
	// most recent model step and would be lost on every session reload.
	stepStatsMu sync.RWMutex
...
func (s *service) Save(ctx context.Context, session Session) (Session, error) {
	...
	s.setStepStats(session.ID, stepStats)
	session = s.fromDBItem(dbSession)
	session.EstimatedUsage = estimatedUsage
	session.Stats = stepStats
...
func (s *service) applyStepStats(session *Session) {
	s.stepStatsMu.RLock()
	session.Stats = s.stepStats[session.ID]
	s.stepStatsMu.RUnlock()
}

func (s *service) setStepStats(sessionID string, stats StepStats) {
	s.stepStatsMu.Lock()
	defer s.stepStatsMu.Unlock()
	if stats == (StepStats{}) {
		delete(s.stepStats, sessionID)
		return
	}
	s.stepStats[sessionID] = stats
}

func (s *service) clearStepStats(sessionID string) {
	s.stepStatsMu.Lock()
	delete(s.stepStats, sessionID)
	s.stepStatsMu.Unlock()
}
